        ...
        <div className="flex justify-center gap-12">
          <div className="flex w-full max-w-xl flex-col gap-8">
            <p className="text-gray-300">
              Pena di sini! Memilih target harian akan membantumu tetap termotivasi saat belajar bahasa. 
              Kamu dapat mengubah target kapan saja.
            </p>
                ...
                {goalXpOptions.map(({ title, xp }, i) => {
                  return (
                    <button
                      key={title}
                      className={[
                        "flex w-full items-center justify-between border-2 p-4  border-opacity-20 first:rounded-t-2xl last:rounded-b-2xl last:border-b-2",
                        xp === localGoalXp
                          ? "border-b-2 border-blue-400 bg-blue-900 text-blue-300"
                          : "border-t-0 border-gray-200 first:border-t-2 text-blue-300 hover:bg-gray-700 hover:bg-opacity-20",
          ...
          <SettingsRightNav selectedTab="Edit Target Harian" />
        </div>
      </div>
    </div>
  );
